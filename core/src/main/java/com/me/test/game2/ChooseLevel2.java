...
public class ChooseLevel2 extends BaseMenuScreen{
	...
	public ChooseLevel2(final TestGame g) {
		super(g);
...
/*	Table page1 = page(1);
	...
	Table page3 = page(LevelMapper.lastStandard+1);
	s.addActor(page3);
	Table page3Labels= pageLabels(LevelMapper.lastStandard+1);
	//page3Labels.setY(300);
	s.addActor(page3Labels);
		
	Table page4 = page(LevelMapper.lastStandard+31);
	s.addActor(page4);
	Table page4Labels= pageLabels(LevelMapper.lastStandard+31);
...
outer:		for(int i = 0; i<height; i++){
					...
					for(Actor child : t.getChildren()){
						if(child.getName()!=null && child.getName().startsWith("image")){
							...
							String choiceStyle;
								 ...
								 if(achievement>0){
									 choiceStyle = new StringBuilder("choice_extra").append(achievement).toString();
								 }
								 else{
									 choiceStyle = new StringBuilder("choice_extra").toString();
								 }
	...
	ClickListener createStartGameListener(final boolean inactive, final int c, final boolean extra){
